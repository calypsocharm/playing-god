...
function renderPanels() {
  ...
  // Legends: what the village keeps telling, how often, and who is presently itching to go and look.
  const lg = (s.legends || []).slice().sort((a, b) => (b.told - a.told) || (b.day - a.day));
  const legendLine = lg.length
    ? `<br>Legends at the fire: ${lg.slice(0, 3).map(l => `<b style="color:var(--warm)">"${esc(l.text.length > 90 ? l.text.slice(0, 88) + '…' : l.text)}"</b> <span class="muted">(day ${l.day}, told ${l.told}×)</span>`).join(' · ')}${lg.length > 3 ? ` <span class="muted">and ${lg.length - 3} more</span>` : ''}${(s.wondering || []).length ? `<br><span style="color:var(--warm)">${esc(s.wondering.join(', '))}</span> ${s.wondering.length === 1 ? 'has' : 'have'} just heard one told and ${s.wondering.length === 1 ? 'wants' : 'want'} to go and see for ${s.wondering.length === 1 ? 'themselves' : 'themselves'}.` : ''}`
    : '<br><span class="muted">Nothing has happened yet that the village tells twice.</span>';
  ...
  $('villageSummary').innerHTML = ended + `${alive.length} alive · ${s.weather.sky} · hearth wood ${s.hearth.wood}${coming}${cardLine}${rivalLine}${legendLine}<br>` +
...
function renderInspector() {
  ...
  $('inspector').innerHTML = `
    ...
    ${a.beliefWord ? `<div class="muted" style="margin-top:4px">Believes <b style="color:${a.belief > 0.15 ? 'var(--good)' : a.belief < -0.15 ? 'var(--bad)' : 'var(--dim)'}">${esc(a.beliefWord)}</b> <span class="muted">(${a.belief > 0 ? '+' : ''}${a.belief}) · it tilts what the day gives them and how they read people</span>${(a.wears || []).length ? ` · wears <b style="color:var(--warm)">${a.wears.map(k => esc(state.itemSpecs?.[k]?.label || k)).join(', ')}</b>` : ''}</div>` : ''}
