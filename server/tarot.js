...
const MINOR = {
  ...
  cups: {
    ...
    Five:   (c) => { const dead = c.w.agents.filter(x => !x.alive); const a = pick(c.alive()); if (!a || !dead.length) return 'No one to grieve; the card passes.'; const d = pick(dead); a.grief = a.grief || []; a.grief.push({ name: d.name, id: d.id, intensity: 0.5, day: c.w.day }); c.remember(a, `${d.name} was in your dream and you woke with the loss of them fresh.`, 0.9); return `${a.name} wakes grieving ${d.name} as if it were the first day.`; },
...
const MAJOR = [
  ...
  ['The Hanged Man', 'waiting, upside down', (c) => { c.w.paused = true; return 'Time stops until the sky moves it again.'; }],
