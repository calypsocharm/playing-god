...
export function whyLines(w) {
  ...
  const rich = living.filter(a => (a.inv.plenty || 0) > 0); if (rich.length) out.push(`${rich.map(a => a.name).join(', ')} ${rich.length === 1 ? 'wears' : 'wear'} the necklace of plenty and ${rich.length === 1 ? 'is' : 'are'} not afraid of tomorrow.`);
