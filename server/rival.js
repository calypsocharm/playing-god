...
// Counted for the far fire's Creator, and reset when their season is weighed (see fire.js).
const bump = (w, k, n = 1) => { const r = w.rival; if (!r) return; r.season = r.season || {}; r.season[k] = (r.season[k] || 0) + n; };
export const moodWord = (m) => m > 0.4 ? 'friendly' : m > 0.1 ? 'warm' : m > -0.2 ? 'wary' : m > -0.5 ? 'cold' : 'hostile';
export const log = (w, text) => { w.rival.log.push({ day: w.day, text }); if (w.rival.log.length > 30) w.rival.log.shift(); };
...
export function nightly(w, c) {
  ...
  if (r.food <= 0) { r.food = 0; r.hungryNights += 1; if (r.hungryNights >= 6 && r.people > 3) { r.people -= 1; r.hungryNights = 0; bump(w, 'deaths'); log(w, 'One of them died hungry.'); } }
  else r.hungryNights = 0;
  if (r.food > 6 && r.people < 20 && Math.random() < 0.03) { r.people += 1; bump(w, 'born'); log(w, 'A child born, or a stranger taken in.'); }
  // Mood drifts home; scarcity sours it.
  r.mood += (0 - r.mood) * 0.01;
  if ((w.deer || 0) < 3) r.mood -= 0.015;
  r.mood = B.clamp(r.mood, -1, 1);
  r.trader = null;
  // If someone holds this fire, what they said goes, ahead of mood and hunger. Held beats sent.
  if (r.willHold) { r.willHold = false; r.willRaid = false; }
  else if (r.willRaid) { r.willRaid = false; raid(w, c); }
  // Trade, when they are warm enough and it has been a while, or the sky asked.
  else if ((r.mood > -0.25 && w.day - r.lastTrade > 8 && Math.random() < 0.22) || r.parley === w.day - 1) trade(w, c);
...
function trade(w, c) {
  ...
  r.lastTrade = w.day; r.trades += 1; bump(w, 'trades'); r.trader = { day: w.day, name: r.leader, sold, bought };
...
function raid(w, c) {
  ...
  if (defence >= r.strength) {
    r.strength = Math.max(1, r.strength - 1); r.people = Math.max(2, r.people - 1); r.mood = B.clamp(r.mood - 0.1, -1, 1); bump(w, 'repelled'); bump(w, 'deaths');
...
export function send(w, a, n, c) {
  const r = ensure(w);
  if (r.seen == null) { c.remember(a, 'You carried food to the edge for people you have only heard of, and left it on a stone.', 0.4); }
  ...
  a.inv.food -= n; r.food += n; r.sent += n; bump(w, 'sent', n);
...
export function parley(w, c) {
  const r = ensure(w);
  if (r.seen == null) return { error: 'the village has not seen another fire yet' };
...
export function snapshot(w, a) {
  const r = w.rival; if (r?.seen == null) return null;
...
export function felt(w, a) {
  const r = w.rival; if (r?.seen == null) return [];
...
export function publicState(w) {
  ...
  return { name: r.name, leader: r.leader, x: r.x, y: r.y, seen: r.seen, held: r.seen != null && r.god?.named != null ? { name: r.god.name, attention: r.god.attention, max: r.god.max } : null, people: r.seen != null ? r.people : null, mood: r.seen != null ? moodWord(r.mood) : null, moodN: r.seen != null ? +r.mood.toFixed(2) : null, food: r.seen != null ? Math.round(r.food) : null, strength: r.seen != null ? r.strength : null, trader: r.trader, trades: r.trades, sent: r.sent, raids: r.raids.slice(-6), log: r.log.slice(-8), lastTrade: r.lastTrade, lastRaid: r.lastRaid };
}
